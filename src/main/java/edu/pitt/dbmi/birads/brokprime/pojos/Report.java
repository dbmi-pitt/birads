...
public class Report {
	...
	public void removeBadNumbers() {
		...
		if (getBody() != null) {
			...
			// kjm; removed outline scrubber 03/03/15
			// setBody(getBody().replaceAll("\\n[\\t ]*\\(?[0-9]+[\\.:\\)]",
			// "\n")); // numbers
			// used
			// for
		...
		if (getImpression() != null) {
			...
			setBody(getImpression().replaceAll(
					"\\b([0-9]+(\\.[0-9]+)?\\s*x\\s*)+([0-9]+(\\.[0-9]+)?)?",
					"")); // dimensions ( 2x4x5 or 1 x 3 x 4
	...
	public void assignBIRADS(int debug) throws IOException {
		...
		while ((line = r.readLine()) != null) {
			...
			Pattern pBIRADSa = Pattern
					.compile(
							".*\\b(left|right|overall|combined|bilateral)\\b.{0,30}?\\b(birads|bi-rads|bi rads|category)\\b.{0,10}?\\b([0-6]{1}|i|ii|iii|iv|v|vi)\\s*([abc]?)\\b.*",
							Pattern.DOTALL);

			Pattern pBIRADSb = Pattern
					.compile(
							".*\\b(left|right|overall|combined|bilateral)\\b.{0,30}?\\b([0-6]{1}|i|ii|iii|iv|v|vi)\\s*([abc]?)\\b.{0,10}?\\b(birads|bi-rads|bi rads|category)\\b.*",
							Pattern.DOTALL);

			// kjm; changed the distance of arbitrary characters after a bi-rads
			// anchor form 10 to 15
			Pattern pBIRADSc = Pattern
					.compile(
							".*\\b(birads|bi-rads|bi rads|category)\\b.{0,15}?\\b([0-6]{1}|i|ii|iii|iv|v|vi)\\s*([abc]?)\\b.{0,30}?\\b(left|right|overall|combined|bilateral)\\b.*",
							Pattern.DOTALL);

			Pattern pBIRADSd = Pattern
					.compile(
							".*\\b([0-6]{1}|i|ii|iii|iv|v|vi)\\s*([abc]?)\\b.{0,10}?\\b(birads|bi-rads|bi rads|category)\\b.{0,30}?\\b(left|right|overall|combined|bilateral)\\b.*",
							Pattern.DOTALL);
	...
	public void assignBIRADSAllowNullLateralityDescription(int debug)
		...
		while ((line = r.readLine()) != null) {
			...
			Pattern pBIRADSa = Pattern
					.compile(
							".*\\b(birads|bi-rads|bi rads|category)\\b.{0,10}?\\b([0-6]{1}|i|ii|iii|iv|v|vi)\\s*([abc]?)\\b.*",
							Pattern.DOTALL);
			...
			if (mBIRADSa.matches()) {
				...
				setNonspecificBIRADS(parseStringToInteger(mBIRADSc.group(1)));

				setNonspecificSubCategory(mBIRADSc.group(2));
